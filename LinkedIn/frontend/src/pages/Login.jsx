...
const Login = () => {
  ...
  return (
    <div className='w-full h-screen bg-[white] flex flex-col justify-start gap-[10px] items-center'>
        ...
        <form action="" onSubmit={handleSignIn} className='w-[90%] max-w-[400px] h-[600px] md:shadow-xl flex flex-col justify-center p-[15px] gap-[10px]'>
            <h1 className='text-gray-800 font-semibold text-[30px] mb-[30px]'>Sign In</h1>
            ...
            <button className='w-[100%] h-[50px] rounded-full text-white bg-[#1dc9fd] mt-[40px]' disabled={loading}>{loading?"Loading...":"Sign In"}</button>
            <p className='text-center cursor-pointer' onClick={()=>navigate("/signup")}>Want to create a new account ? <span className='text-[#07afe2]'>Sign Up</span></p>
        </form>
    </div>
  )
}
